...
**The MCP `test` tool has no `--collect-coverage-from` parameter**, so on its own it can only run the `imports` gate. `app/very_good.yaml` closes that: the CLI reads the file's `test` section as the default for every flag not passed on the command line, the MCP tool included, so a bare `very_good test` from any process is the full gate. The explicit flags on the commands in this plan stay, because they agree with the file and a reader sees the gate without opening it.
