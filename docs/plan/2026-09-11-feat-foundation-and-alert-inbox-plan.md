...
This plan covers the foundation and the first shippable slice of Keeper: scaffold the Flutter app, hand-write the domain and the baseline evaluator by example, hand-write `AlertInboxBloc` by example with its tests, run the first green gate and capture its additions as the evidence, then ship the alert inbox screen against seeded local data.
...
2. **The hand-written core is evidence and it expires.** Once an agent has written into the domain, the claim "I wrote this by hand and can explain every line" cannot be made about it. The order is one-way.
...
**Cold writing was retracted on Sep 13, 2026, and replaced with guided scaffolding.** Jamie is new to Dart and has nine days, and "write the class" is not enough direction for a first file in a new language. `docs/learning/` now holds a complete, tested example package in a different domain (servers, not hives) with the same shapes the plan asks for, one document per hand-written phase, and folded hints. Jamie reads the example and writes the keeper version by hand against this plan's spec. Reading stays open: dart.dev, pub.dev, the template's counter feature, and the plugin's `bloc`, `testing` and `layered-architecture` skill files. Asking an agent a question is fine while the answer is prose or a pointer into the examples. The line that remains: no agent writes a file under `app/lib/domain/`, `app/lib/repositories/` or `app/lib/alert_inbox/` in Phases 2 to 4, because the claim the repository makes is that Jamie can explain every line of those on demand.
...
- **Status:** Not started
- **Owner:** **Jamie, by hand, guided by `docs/learning/02-domain-types.md`. No agent writes these files.**
...
- **Read first:** `docs/learning/02-domain-types.md`, then the eight decisions through the incident paragraphs, the `Alert` and anomaly shapes under State shape, and `docs/original-plan.md` section 2 for each type's fields. Where they differ this plan wins: the fifth metric is `tilt`, not motion, and nothing this plan cut or deferred is written.
...
<details>
<summary><strong>Hints for Phase 2. Open only if stuck.</strong> The spec above is complete without them.</summary>

- Start with `Metric` and the two extension types. Nothing depends on them and everything imports them.
- A `DateTime` cannot be `const`, so `Reading` and `Alert` keep `const` constructors but their call sites in tests are `final` with a fixed `DateTime.utc(...)`.
- `List` and `Set` fields compare element by element inside `props`; a `Map` does not, so no `Map` in a domain type or a state.
- If the analyzer says a constructor is not `const`, a field is missing `final` or a nested value is not `const`.
- The nine-cell severity table is a static method on the enum and nine one-line tests, one per cell, stated in sigma and readings.
- `docs/learning/02-domain-types.md` walks each construct with a tested example in a different domain.
...
- **Status:** Not started
- **Owner:** **Jamie, by hand, guided by `docs/learning/03-the-evaluator.md`. No agent writes these files.**
...
- **Read first:** `docs/learning/03-the-evaluator.md`, then decisions 1 to 6, the bold paragraphs after the table through the sigma floor, the severity table, and the `EvaluationResult` block under State shape.
...
- **Status:** Not started
- **Owner:** **Jamie, by hand, guided by `docs/learning/04-repository-and-bloc.md`. No agent writes these files.**
