...
6. [PROCESS] Always trigger CodeRabbit on every pull request by commenting `@coderabbitai review` right after `/create-pr`, and check that it produced a review rather than a skip notice, because automatic reviews are off for public repos under 10 stars and the three-reviewer comparison is the point of the repo. `.coderabbit.yaml` excludes the platform directories so the review stays on the Dart, CI and docs (Jamie, Sep 13, 2026).
7. [TOOL] Always wait on external state with a tool built for it, never an ad-hoc sleep loop: CI with `gh run watch <run-id> --exit-status` in the background after confirming the run exists, anything without a watch command (a bot review, a deploy) with the Monitor tool's until-loop and a bounded timeout, and report on the completion notification, because hand-rolled sleep loops finished without reporting three times on Sep 13, 2026 and Jamie had to check the page himself (Jamie, Sep 13, 2026).
8. [PROCESS] Never write a file under `app/lib/domain/`, `app/lib/repositories/` or `app/lib/alert_inbox/` for Phases 2 to 4, and never answer a question about them with code that could be pasted in; teach through the collapsed constructs blocks in the build plan instead, and put any new teaching material in the phase that needs it rather than in a new document, because the cold-writing claim was retracted on Sep 13, 2026 for a narrower one, Jamie wrote those files by hand against worked examples and can explain every line, and the first attempt at the fix added an 865-line example package that CI would never have analyzed (Jamie, Sep 13, 2026).
