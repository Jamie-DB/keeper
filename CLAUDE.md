...
8. [PROCESS] Never write a file under `app/lib/domain/`, `app/lib/repositories/` or `app/lib/alert_inbox/` in Phases 2 to 4, and never answer a question about them with code that could be pasted in, because the claim the repository makes is that Jamie wrote those files by hand and can explain every line. The plan's own phase blocks carry the guidance that replaced cold writing (Jamie, Sep 13, 2026).
